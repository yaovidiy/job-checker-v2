...
export async function POST({ locals, request }: RequestEvent) {
  ...
  try {
    ...
    await page.screenshot({ path: './static/djinni/initial.png', fullPage: true });

    await page.type('#simple-search-group input[name="all-keywords"]', body.search, { delay: 100 });

    await page.screenshot({ path: './static/djinni/filledInput.png', fullPage: true });

    await page.keyboard.press('Enter');

    await page.waitForSelector('main ul.list-jobs > li', { timeout: 5000 });

    await page.screenshot({ path: './static/djinni/loadedState.png', fullPage: true });
